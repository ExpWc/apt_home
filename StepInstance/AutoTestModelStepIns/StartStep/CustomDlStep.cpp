...
void *threadSimpoleExecute(void *pParam)
{
	...
	//<< 1
	while ((pCustomDlStep->m_statusFlag != STEP_STOP) && (pCustomDlStep->m_statusFlag != STEP_NULL) && (pCustomDlStep->m_statusFlag != STEP_INIT))
	{
...
int CCustomDlStep::initLibrary()
{
...
#ifdef AUTO_LOG
		string errorMsg = string("initLibrary : ") + string(errMsg);
		STEPLOG(LOG_ERROR, errorMsg);
#endif
	...
	if (dlDoOneStep == NULL)
	{
		...
		// STEPLOG(LOG_ERROR, "Undefined reference autoTestDoOneStep.\n");
...
int CCustomDlStep::run()
{
	...
	{
#ifdef AUTO_LOG	
		STEPLOG(LOG_ERROR, "pthread_create failed \n");
#endif
		REPORT(REPORT_ERROR, "pthread_create failed.\n");
		return -1;
	}
