...
slMessage *_slMsgSvcCreateMsg(slMsgManager *msgMgr, 
                              ...
                              slMsgQueueId queueId)
{
    return _slMsgSvcCreateMsgWithId(
        msgMgr, 
        data, 
        dataSize, 
        queueId, 
        (++(msgMgr->fQueues[queueId]->_nextMsgId)) + (queueId << 16));
}
...
{
    return _slMsgSvcCreateMsg(mgr, msg->fData, msg->fDataSize, msg->fQueueId);
}
...
{
    slMsgQueue *q = mgr->fQueues[queueId];
    return _slMsgSvcCreateMsg(mgr, data, q->fDataSize, queueId);
}
