...
#define S_FUNCTION_NAME  gvar_getArray
...
static void mdlInitializeSizes(SimStruct *S)
{
   ...
   /* ssSetInputPortDirectFeedThrough(S, 0, 1);*/
    ...
    varLength = (int)mxGetPr(ssGetSFcnParam(S,1))[0];/* get the length of data. */
    //varLength = (int)ssGetSFcnParam(S,0)[1];
    printf("====get varLength:%d\n", varLength);
    
    //ssSetOutputPortDimensionInfo( S, 0, DYNAMIC_DIMENSION); 
    //ssSetOutputPortWidth(S, 0, DYNAMICALLY_SIZED);    
    ssSetOutputPortWidth(S, 0, varLength);
...
#endif
    printf("====>>gvar_get path:%s\n", S->path);
    printf("====>>gvar_get modelName:%s\n", modelName);
    printf("====>>gvar_get offset:%d ret:%d varlen:%d\n",offset,offsetFlag,varLength);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
   ...
   int i;
   char a = 1;
   unsigned char *recvdata;/* the pointer of receive buffer.*/
   
   //int_T     yWidth = ssGetOutputPortWidth(S, 0);  
   //printf("=========TST=======gvar_get====>>yWidth:%d\n", yWidth);
   //const uint8_T  *y = ssGetOutputPortSignal(S,0);
   
   recvdata = (unsigned char*)ssGetOutputPortSignal(S,0);
   //varLength = strlen(recvdata);
   
   #if 0
   printf("=========TST=======gvar_get====>>varLength:%d\n", varLength);
   ...
   #if 0
   //    
   memcpy((char *)recvdata, &a, 1);
   a = 2;
   memcpy((char *)recvdata+1, &a, 1);
   a = 3;
   memcpy((char *)recvdata+2, &a, 1);
   a = 4;
   memcpy((char *)recvdata+3, &a, 1);
   a = 5;
   memcpy((char *)recvdata+4, &a, 1);
   printf("=========gvar_get====>>recvdata===>>");
   for(i = 0; i < 5; i++)
   {
        printf("%02x  ", recvdata[i]);   
   }
   ...
   /* const real_T *u = (const real_T*) ssGetInputPortSignal(S,0);*/
    ...
    if(offsetFlag == OFFSET_CONFIG_OK)
    {
          ...
          getVarData(offset,(char *)recvdata, varLength);
    }
