                         ...
                         void *memBlock)
{
    ...
    if (chunk->fBlockSize > MSG_POOL_MAX_CHUNK_SIZE)
        chunk->fBlockSize = MSG_POOL_MAX_CHUNK_SIZE;
...
void *__slmsg_MemPool_Alloc(__slmsg_MemPool* pool, ulong_T nbytes)
{
    ...
    if (NULL == pool->fFreeListHead) {
        ...
        if (pool->fCanMalloc) { /* Grow the memory pool */
            ...
            if (newBlockSize > MSG_POOL_MAX_CHUNK_SIZE)
                newBlockSize = MSG_POOL_MAX_CHUNK_SIZE;
