...
const char caseId[64] = "afd8b38e-a026-4c01-8aac-947dfa2de763";
//const char caseId1[64] = "7fb0c577-de54-44fc-9686-f0009f4cb2c4";
int main()
{
	...
	cout << "-===>> 21:Manualstart; 22:optManual; 23:ManualStop" << endl;
    string cmd;
    int ret = 0;
    while (cin>>cmd)
    {
      ...
      if (strcmp(cmd.c_str(), "4") == 0)//storage
        {
            ...
            string str;
            //str = string(GetXmlData(xmlFileName));
            str = string(GetXmlData("bin/StorageInfo.xml"));
            //printf("===>>%s\n", str.c_str());
      ...
      if (strcmp(cmd.c_str(), "5") == 0)//curve
        {
            ...
            string str;
            //str = string(GetXmlData(xmlFileName));
            str = string(GetXmlData("bin/CurveInfo.xml"));
            //printf("===>>%s\n", str.c_str());
...
/*
            memset(testExecute.caseExecute[1].caseID, 0, 64);
            sprintf(testExecute.caseExecute[1].caseID, caseId1);
            testExecute.caseExecute[1].caseExecuteTimes = 1;
            testExecute.caseExecute[1].interval = 0;
*/
        ...
        if (strcmp(cmd.c_str(), "8") == 0)//statuss
        {
         ...
         if (strcmp(cmd.c_str(), "21") == 0)//manualstart
		{
        ...
        if (strcmp(cmd.c_str(), "22") == 0)//opt
		{
			TY_STEP_STATUS stepInfo;
			//TY_SIM_RESCOURCE_INFO *pRes=new TY_SIM_RESCOURCE_INFO;xml/ChannelConfig1109.xml
			//char channelConfig[64] = "channel config\n";/home/V1.0.0/HardwareManager/ChannelConfig.xml
			ret = optManualTestOneStep(GetXmlData("bin/CaseInfo.xml"), 	&stepInfo);
			//ret = setChannelConfig("xml/ChannelConfig1109.xml");

			printf("===>>%d\n",ret);
		}
		if (strcmp(cmd.c_str(), "23") == 0)//manualstart
		{
			//TY_STEP_STATUS stepInfo;
			//TY_SIM_RESCOURCE_INFO *pRes=new TY_SIM_RESCOURCE_INFO;xml/ChannelConfig1109.xml
			//char channelConfig[64] = "channel config\n";/home/V1.0.0/HardwareManager/ChannelConfig.xml
        ...
        if (strcmp(cmd.c_str(), "15") == 0)//getModelVar
        {
            ...
            modelInfo.num = 1;
            sprintf(modelInfo.fullPath[0], "test1/Shared Memory Get1");
        ...
        if (strcmp(cmd.c_str(), "18") == 0)//setModelParam
        {
            ...
            sprintf(modelInfo.fullPath[0], "test1/Gain");
            sprintf(modelInfo.modelName, "test1");
            modelInfo.data[0] = 3.7;
            ret = setModelParam(&modelInfo, 1);
            printf("--set-->%d data:%lf\n", ret,modelInfo.data[0]);
        }
